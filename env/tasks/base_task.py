...
@register
class BaseTask(NullTask):
    def __init__(self, env: LeggedRobotEnv):
        super(BaseTask, self).__init__(env)
        self.env = env
        self.cmd_id = 0
        self.rew_names = None
        ...
        self.phase_modulator = PhaseModulator(time_step=env.dt, num_envs=self.num_envs, num_legs=self.num_legs,
                                              device=self.device)
    ...
    def reset(self, env_ids):
        ...
        self.joint_pos_error = self.current_joint_act - self.joint_pos
        self.static_flag = torch.where(torch.norm(self.commands[:, :3], dim=1, keepdim=True) < 0.12, False, True).float()
        if self.cfg.terrain.curriculum:
    ...
    def observation(self, pred_data=None):
        self.obs_buf_pure = self.pure_observation(pred_data)
    ...
    def pure_critic_observation(self):
        obs_buf = torch.cat([
            ...
            self.env.foot_frc.clip(min=0., max=500.) * 0.01,
        ], dim=1)
        return obs_buf

    def pure_observation(self, pred_data=None):
    ...
    def action(self, net_out):
        ...
        self.current_joint_act = torch.clip(self.current_joint_act, self.joint_action_limit_low,
    ...
    def _resample_commands(self, env_ids):
        ...
        case_num = []
        if self.command_cfgs["lin_vel_x_range"][1] > 0:
            case_num.append(0)
        elif self.command_cfgs["lin_vel_y_range"][1] > 0:
            case_num.append(1)
        chosen = random.choice(case_num)

        self.commands[env_ids, :] = torch.zeros(len(env_ids), self.cfg.command.num_commands, dtype=torch.float,
                                                device=self.device, requires_grad=False)
        if chosen == 0:
            self.commands[env_ids, 0] = torch_rand_float(self.command_cfgs["lin_vel_x_range"][0],
                                                         self.command_cfgs["lin_vel_x_range"][1],
                                                         (len(env_ids), 1), device=self.device).squeeze(1)
            self.commands[env_ids, 1] = torch_rand_float(self.command_cfgs["lin_vel_y_range"][0],
            ...
            self.commands[env_ids, 1] = torch_rand_float(self.command_cfgs["lin_vel_y_range"][0],
                                                         self.command_cfgs["lin_vel_y_range"][1],
                                                         (len(env_ids), 1), device=self.device).squeeze(1)
        self.commands[env_ids, 2] = torch_rand_float(self.command_cfgs["ang_vel_yaw_range"][0],
                                                     self.command_cfgs["ang_vel_yaw_range"][1],
                                                     (len(env_ids), 1), device=self.device).squeeze(1)
        if self.cfg.command.heading_command:
            self.commands[env_ids, 3] = torch_rand_float(self.command_cfgs["heading_range"][0],
        ...
        self.commands[env_ids, 0:1] *= torch.where(torch.norm(self.commands[env_ids, 0:1], dim=1, keepdim=True) < 0.15,False, True).float()
        self.commands[env_ids, 2:3] *= torch.where(torch.norm(self.commands[env_ids, 2:3], dim=1, keepdim=True) < 0.15,False, True).float()
    ...
    def terminate(self):
        ...
        if self.env.render or self.env.epochs > 1 or self.env.tcn_name is not None:
            ...
            done = con_over | pos_over | twist_over | time_out  | jact_over  # jpos_over
        return done
    ...
    def reward(self):
        ...
        base_heit_rew = torch.exp(-70 * (self.env.base_pos[:, [2]] - 0.44) ** 2)

        balance_rew = 0.5 * (base_heit_rew * torch.exp(
            -torch.clip(4. / lin_vel_x_norm, min=2, max=8.) * torch.norm(self.env.base_euler[:, :2], dim=-1, keepdim=True)) + 1.)

        forward_vel_rew = torch.exp(-torch.clip(6. / lin_vel_x_norm, min=2., max=10.) * (
                self.commands[:, [0]] - self.env.base_lin_vel[:, [0]]) ** 2) * balance_rew
        lateral_vel_rew = torch.exp(-torch.clip(4. / lin_vel_x_norm, min=3., max=15.) * torch.norm(self.env.base_lin_vel[:, [1]], dim=1, keepdim=True) ** 2)

        yaw_rate_rew = torch.exp(-8. * (0 - self.env.base_ang_vel[:, [2]]) ** 2) * balance_rew
        yaw_rate_rew += -0.2 * (self.env.base_ang_vel[:, [2]]) ** 2

        lateral_vel_rew += -0.3 / lin_vel_x_norm * torch.norm(self.env.base_lin_vel[:, [1]], dim=1, keepdim=True) * self.static_flag

        ang_vel_rew = torch.exp(
            -torch.clip(1.5 / lin_vel_x_norm, min=0.7, max=6.) * torch.norm(self.env.base_ang_vel[:, :2], dim=1,
                                                                            keepdim=True) ** 2)
        ...
        vertical_vel_rew = torch.exp(-torch.clip(4. / lin_vel_x_norm, min=2., max=10.) * torch.norm(self.env.base_lin_vel[:, [2]], dim=1,
                                                                                                    keepdim=True) ** 2)
        vertical_vel_rew -= 0.1 / lin_vel_x_norm * torch.norm(self.env.base_lin_vel[:, 1:], dim=1, keepdim=True) * self.static_flag
        ...
        foot_clear_rew = torch.sum(torch.logical_and(swing_foot_index, swing_foot_index), dtype=torch.float, dim=1, keepdim=True) / self.num_legs

        foot_support_rew = torch.sum(torch.logical_and(support_foot_index, support_foot_index), dtype=torch.float, dim=1, keepdim=True) / self.num_legs
        foot_support_rew *= self.static_flag
        foot_clear_rew *= self.static_flag

        foot_heit_score = 40. * torch.clip(self.foot_height, min=0.0, max=0.06)
        foot_height_rew = torch.sum(swing_foot_index * foot_heit_score, dim=1, keepdim=True).clip(max=2.) * self.static_flag
        ...
        feet_contact_frc_rew = -torch.norm(self.env.foot_frc * swing_foot_index, dim=1, keepdim=True) * self.static_flag
        ...
        foot_slip_rew = 2. * (lin_vel_x_norm * torch.sum(
            (self.env.foot_vel.view(self.num_envs, self.num_legs, -1)[:, :, 0]) * self.commands[:, [0]].sign() * swing_foot_index,
        ...
        foot_slip_rew += -0.3 / lin_vel_x_norm * torch.norm(
            0.1 * torch.norm(self.env.foot_vel.view(self.num_envs, self.num_legs, -1)[:, :, :2], dim=-1) / clip_foot_h * support_foot_index, dim=1,
            keepdim=True) * self.static_flag
        ...
        action_constraint_rew = -0.1 * torch.clip(1. / lin_vel_x_norm, 0, 1.) * torch.norm((self.current_joint_act - self.ref_joint_action), dim=1,
                                                                                           keepdim=True)
        ...
        sa_constraint_rew = -0.1 * torch.clip(1. / lin_vel_x_norm, min=0., max=1.) * torch.norm(self.current_joint_act - self.ref_joint_action, dim=1,
                                                                                                keepdim=True) ** 2 * self.static_flag
        ...
        joint_pos_error_rew = - 0.4 * torch.clip(1. / lin_vel_x_norm, min=0., max=1.) * torch.norm((self.current_joint_act - self.env.joint_pos), dim=1,
                                                                                                   keepdim=True) ** 2
        ...
        pmf_rew = -0.02 * torch.clip(1. / lin_vel_x_norm, min=0., max=1.) * torch.norm(
            ...
            dim=1, keepdim=True)
        pmf_rew += -0.5 * torch.clip(1 / lin_vel_x_norm, 0, 1.) * torch.norm(self.net_out_history[-1][:, :self.num_legs] * support_foot_index, dim=1,
                                                                             keepdim=True) ** 2
        pmf_rew *= self.static_flag

        net_out_val_rew = -0.4 * torch.clip(1. / lin_vel_x_norm, min=0., max=1.) * torch.norm(self.net_out_history[-1][:, self.num_legs:], dim=1,
                                                                                              keepdim=True) ** 2
        ...
        rew_dict = dict(
            ...
            sa_const=sa_constraint_rew * balance_rew * 0.05,
            ...
            foot_vz=foot_vz_rew * 0.1 * balance_rew,
            foot_acc=foot_acc_rew * balance_rew * 0.05,
            foot_sft=foot_soft_rew * 0.7 * balance_rew,
