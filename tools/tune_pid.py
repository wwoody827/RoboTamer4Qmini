...
def tune_pid(args):
    ...
    #
    stiffness = {'hip_yaw': 55., 'hip_roll': 105., 'hip_pitch': 75., 'knee': 45., 'ankle': 30.}  # [N*m/rad]
    ...
    s_nsteps_ago = -90 # Data from the Qmini_V0, for Qmini_V1 (the release version), it should be about -25 (means delays of 25ms )
    ...
    debug_dir = join('experiments/tune_pid/real', 'pid1.5hz')  # sin_test birl10_on_rack
    ref_act = torch.tensor([0.4, -0.1, -1.5, 1., -1.3, -0.4, 0.1, 1.5, -1., 1.3], dtype=torch.float)

    os.makedirs(debug_dir, exist_ok=True)
    if mode == 'real':
        xl = pd.read_csv(join(debug_dir, 'general.txt'), sep='\t+', header=None).values[2:, :].astype(float)
        joint_act_real = xl[:, :num_dofs][:, :num_dofs]
        joint_pos_real = xl[:, num_dofs:num_dofs + num_dofs][:, :num_dofs]
        joint_vel_real = xl[:, 2 * num_dofs:2 * num_dofs + num_dofs][:, :num_dofs]
        ts_real = np.linspace(0, T, len(joint_act_real))
        joint_act_inc_real = joint_act_real.copy()
        joint_act_inc_real[1:, :] = (joint_act_inc_real[1:, :] - joint_act_inc_real[:-1, :]) / 0.01
        joint_tau_real = kp_real * (joint_act_real - joint_pos_real) + kd_real * (joint_act_inc_real * enable_d_ctrl - joint_vel_real)
    ...
    for t in ts:
        if mode == 'reset':
            ...
            for i in range(num_dofs):
                act[i] = ref_act[i] + math.sin(2. * (2. * pi) * t) * 0.2
        elif mode == 'real':
            for i in range(num_dofs):
                act[i] = joint_act_real[count, i]
                act_inc[i] = joint_act_inc_real[count, i]
        count += 1
    ...
    # draw a picture
    num = min(len(ts / sim_params.dt), len(ts_real / sim_params.dt)) if mode == 'real' else len(ts / dt)
    colors = ['r', 'g', 'b']
